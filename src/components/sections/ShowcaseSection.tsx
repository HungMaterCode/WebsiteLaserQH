      ...
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div ref={titleRef} initial={{ opacity: 0, y: 30 }} animate={{ opacity: titleInView ? 1 : 0, y: titleInView ? 0 : 30 }} transition={{ duration: 0.6 }} className="text-center mb-10">
          ...
          <h2 className="font-orbitron mb-5" style={{ fontSize: 'clamp(2.2rem, 5vw, 3.5rem)', fontWeight: 800, lineHeight: 1.1 }}>
            ...
            <span className="block" style={{ color: 'var(--neon-green)' }}>Chúng Tôi Tự Hào</span>
          </h2>
              ...
              >
                ...
                <div className="relative aspect-[4/3] overflow-hidden bg-[#0A0F1A]">
                  ...
                  <div className="absolute inset-0 bg-black/30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                    <div className="w-16 h-16 rounded-full border flex items-center justify-center backdrop-blur-md transition-transform duration-300 scale-90 group-hover:scale-100" style={{ borderColor: 'var(--neon-cyan)', backgroundColor: 'rgba(0, 229, 255, 0.15)' }}>
                      <svg width="22" height="22" viewBox="0 0 24 24" fill="var(--neon-cyan)" stroke="var(--neon-cyan)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ marginLeft: '4px' }}>
                ...
                <div className="p-6 flex flex-col flex-1 z-10 relative">
                  ...
                  {/* Buttons Row */}
                  <div className="flex flex-wrap items-center gap-3 mt-auto w-full">
                    <a href={project.slug === 'samsung-launch-2024' ? 'https://facebook.com' : '#'} target="_blank" rel="noreferrer" className="flex-1 min-w-[120px] justify-center px-4 py-2.5 rounded-lg text-[0.8rem] font-medium flex items-center gap-2 transition-colors" style={{ border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.6)', fontFamily: 'var(--font-vietnam)' }}
                      onMouseEnter={(e) => { e.currentTarget.style.color = '#fff'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.3)'; e.currentTarget.style.background = 'rgba(255,255,255,0.03)' }}
                    ...
                    <Link href={`/lien-he?project=${project.slug}`} className="flex-1 min-w-[140px] justify-center px-4 py-2.5 rounded-lg text-[0.8rem] font-bold flex items-center gap-1.5 transition-all" style={{ color: project.color, background: `${project.color}05`, border: `1px solid ${project.color}30`, fontFamily: 'var(--font-vietnam)' }}
                      onMouseEnter={(e) => { e.currentTarget.style.background = `${project.color}15`; e.currentTarget.style.borderColor = `${project.color}60` }}
                      onMouseLeave={(e) => { e.currentTarget.style.background = `${project.color}05`; e.currentTarget.style.borderColor = `${project.color}30` }}
                    >
                      Setup Tương Tự
                      <span className="text-[1.1rem] leading-none mb-[2px]">→</span>
                    </Link>
                  </div>
                </div>
              </motion.div>
